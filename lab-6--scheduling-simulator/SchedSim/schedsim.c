...
{ 
  ...
  int rem_bt[n]; //store burst times 
  int isDone[n]; //store completion status of each process
  ...
  for(int j=0; j < n; j++){
    if(isDone[j] == 0){//process has not been reached yet 
      if(rem_bt[j] > quantum){
        t = t + quantum;
        rem_bt[j] -= quantum;
      }
    ...
    //last iteration --restart if not all are done 
    if(j == (n-1)){
      for(int l; l < n; l++){
        if(isDone[l] == 0){
          done = 0;
...
{ 
    ...
    int current_time = 0;
    int counter = 0;
    while(counter < n){ //Traverse until all process gets completely executed.
      
      //Find process with minimum remaining time at every single time lap.
      int min = 0, j;
      for(j = 1; j < n; j++){
        if(proc_bt[j] < proc_bt[min]){
          min = j;
        }
      }
      ...
      if(proc_bt[min] == 0){
        ...
        //Calculate waiting time for each completed process. wt[i]= Completion time - arrival_time-burst_time
        plist[min].wt = completion_time - plist[min].art - plist[min].bt;
...
{ 
    ...
    if(this_proc->pri < that_proc->pri){
      return 1;
    }
...
{ 
  ...
  //find avegerage waiting time using FCFS scheduling algorithm
